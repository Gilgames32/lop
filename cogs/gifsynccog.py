...
class GifSyncCog(commands.Cog):
    ...
    # sync gifs to cloud
    @app_commands.command(name="gifsync", description="Sync favorite gifs with the cloud")
    @app_commands.describe(gc="gif collection json")
    async def purge_self(self, interaction: discord.Interaction, gc: discord.Attachment = None):
        ...
        if gc is None:
            await interaction.response.send_message(f"Unfinished feature", ephemeral=True, file=discord.File("./gif-collections.json"))
        ...
        await interaction.response.send_message(f"Unfinished feature\nCloud save overwritten", ephemeral=True)
